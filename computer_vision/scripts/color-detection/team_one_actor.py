...
class TeamOneActor(object):
    ...
    def process_image(self, msg):
        ...
        self.ball_binary_image = cv2.inRange(
            self.hsv_image, (100, 120, 110), (160, 256, 256))

        # Make annotations, determine flag
        self.annotate_contours()

        # Use flag (/bump AND ball size in /camera) to determine ball finding
        # or goal navigation
        self.seek_binary(self.ball_binary_image, self.goal_binary_image)

    def annotate_contours(self):
        """
        Visualizes detected objects, identifies if a ball is in frame
        """
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if self.cv_annotated is not None:
                cv2.imshow('TEAM ONE (BLUE): ROBOT ' +
